...
class Home extends Component {
  ...
  async accept(userid) {
    try {
      ...
      let newFriends = this.state.requests.filter((user) => { return hash(user.email) === userid});
      let newInvites = this.state.requests.filter((user) => { return hash(user.email) !== userid});
